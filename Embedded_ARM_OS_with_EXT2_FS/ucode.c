...
// Returns num chars start to '\n' inclusive
int find_line(char line[], char buf[], int bufsize) {
  int i;
  for (i = 0; i < bufsize; i++) {
    if (buf[i] == '\n' || buf[i] == '\0') {
      line[i] = '\0';
...
// // Precondition: str represents a valid 32-bit decimal (base ten) integer
// int atoi(char str[]) {
//   int num = 0, power = 1, i = strlen(str) - 1; // set i to index of last char
...
char * strtok(char * str, char delims[]) {
  static char * pToken;
  // printf("  pToken == %d\n", pToken);
  if (pToken == NULL) { // no more tokens in original string
    return NULL;
  }
  ...
  int i = 0;
  for ( ; str[i] != '\0' && !contains(str[i], delims); i++) {} // find first delim
  // printf("  strtok: i after 1st for== %d\n", i);
  // now str[i] is either a delim or '\0'
  if (str[i] == '\0'){ // no delims found
    pToken = NULL;
    return str;
  }
  else {
    for ( ; contains(str[i], delims); i++) {
      str[i] = '\0'; // replace delim chars with '\0'
    } // find next non-delim
    pToken = &str[i];
    return str;
  }
